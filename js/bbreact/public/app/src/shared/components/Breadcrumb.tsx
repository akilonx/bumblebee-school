	...
	if (path.href === undefined) {
		return (
			<>
				...
				{denominator && <i className="svg-icon mod-chevron mod-right path-denominator" />}
			</>
		);
	} else if (router) {
		return (
			<>
				{path.disabled ? (
					...
					<Link className="path-name link link--hover link--small" to={path.href}>
						{t(path.name)}
					</Link>
				)}
				{denominator && <i className="svg-icon mod-chevron mod-right path-denominator" />}
...
export function generateBreadcrumb(url: string, visualUrl?: string, prefix?: string) {
	if (prefix) {
...
export function extendBreadcrumb(
	...
	const last = breadcrumb.length > 0 ? breadcrumb[breadcrumb.length - 1] : { href: "", name: "" };
