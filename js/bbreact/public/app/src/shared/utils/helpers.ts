...
export function range(
  ...
  if (start + step > end) {
    throw new Error(
      "Cannot create a valid range since start + step is larger than the end"
    );
  }
...
export function findInTree<T, S extends keyof T, U extends keyof T>(
  ...
  while (hayStack.length > 0) {
    ...
    if (node === undefined) {
      throw new Error(
        "[findInTree] the popped node was undefined -- should not happen"
      );
    }
...
export function uniqueValues<
  T,
  K extends TypedKeys<string | number, T> & string
>(list: Array<T>, key: K) {
